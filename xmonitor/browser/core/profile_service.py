...
def is_persistent_browser_profile_dir(profile_dir, persistent_dir):
    profile = os.path.abspath(os.path.expanduser(str(profile_dir or ''))).rstrip('/')
    persistent = os.path.abspath(os.path.expanduser(str(persistent_dir or ''))).rstrip('/')
    return bool(profile and persistent and profile == persistent)


def create_browser_user_data_dir(prefer_persistent, *, persistent_dir, data_dir):
    persistent_dir = os.path.abspath(os.path.expanduser(str(persistent_dir or '')))
    data_dir = os.path.abspath(os.path.expanduser(str(data_dir or '')))
    if prefer_persistent and persistent_dir:
        os.makedirs(persistent_dir, exist_ok=True)
        cleanup_stale_profile_singletons(persistent_dir)
        return persistent_dir
    os.makedirs(data_dir, exist_ok=True)
    return tempfile.mkdtemp(prefix='chromium-profile-', dir=data_dir)


def cleanup_browser_user_data_dir(profile_dir, *, persistent_dir):
    target = os.path.abspath(os.path.expanduser(str(profile_dir or '')))
    if not target:
        return False
    if is_persistent_browser_profile_dir(target, persistent_dir):
        cleanup_stale_profile_singletons(target)
        return False
    try:
