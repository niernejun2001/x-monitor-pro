...
DEFAULT_EMOJI_JOINER_CHARS = {'\u200d', '\ufe0f', '\u20e3'}
...
def contains_emoji_char(ch, deps):
    cp = ord(ch)
    ranges = getattr(deps, 'EMOJI_UNICODE_RANGES', DEFAULT_EMOJI_UNICODE_RANGES)
...
def is_emoji_only_content(content, deps):
    ...
    if not text:
        return False
    has_emoji = False
    joiner_chars = getattr(deps, 'EMOJI_JOINER_CHARS', DEFAULT_EMOJI_JOINER_CHARS)
    for ch in text:
        if ch.isspace() or ch in joiner_chars:
            continue
