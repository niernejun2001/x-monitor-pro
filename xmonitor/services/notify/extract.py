...
def strip_notification_meta_prefix(text):
    ...
    if not raw:
        return ''
    patterns = (
        r'^\s*.+?\s+@[\w_]{1,30}\s*·\s*[^ ]+\s*(?:回复(?:\s*@[\w_]{1,30})*|回复了你|提到了你)\s*',
        r'^\s*@[\w_]{1,30}\s*·\s*[^ ]+\s*(?:回复(?:\s*@[\w_]{1,30})*|回复了你|提到了你)\s*',
        r'^\s*.+?\s*·\s*[^ ]+\s*(?:回复(?:\s*@[\w_]{1,30})*|回复了你|提到了你)\s*',
    )
    cleaned = raw
    ...
    *,
    ...
    def add_candidate(source, text):
        ...
        for variant_source, variant_text in variants:
            ...
            if variant_source == 'tweetText':
                tweet_text_candidates.append(variant_text)
