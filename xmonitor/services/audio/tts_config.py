def build_notify_tts_runtime_payload(deps, include_secrets=True):
    payload = {
        ...
        'notify_tts_access_token_configured': bool(str(deps.DOUBAO_TTS_ACCESS_TOKEN or '').strip()),
        'notify_tts_secret_key_configured': bool(str(deps.DOUBAO_TTS_SECRET_KEY or '').strip()),
    }
    if include_secrets:
