...
def register_notify_reply_routes(app, deps):
    ...
    def notify_reply():
        ...
        if not ok:
            ...
            if scheduled:
                return jsonify(build_retry_payload(flow_err_code, flow_err_detail, retry_at, cur_attempt, schedule_msg)), 202
            payload = build_retry_payload(flow_err_code, flow_err_detail, 0, cur_attempt, f'{err}（{schedule_msg}）', status='err')
