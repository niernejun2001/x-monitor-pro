...
LPK_DIR="${LPK_DIR:-$ROOT_DIR/lazycat-lpk-vnc}"
MANIFEST_PATH="${MANIFEST_PATH:-$LPK_DIR/lzc-manifest.yml}"
DOCKERFILE="${DOCKERFILE:-$ROOT_DIR/Dockerfile.vnc}"
SRC_IMAGE_REPO="${SRC_IMAGE_REPO:-registry.cn-hangzhou.aliyuncs.com/shoxk8s/x-monitor-pro-vnc}"
APP_PACKAGE="${APP_PACKAGE:-cloud.lazycat.app.xmonitor-vnc}"
...
main() {
  ...
  log "构建镜像(Dockerfile.vnc): $IMAGE_TAG"
  docker build -f "$DOCKERFILE" -t "$IMAGE_TAG" -t "$IMAGE_LATEST" "$ROOT_DIR"
