    ...
    for attempt in range(1, max_attempts + 1):
        with browser_lock:
            ...
            if global_browser_dir:
                try:
                    shutil.rmtree(global_browser_dir, ignore_errors=True)
                except Exception:
                    pass
                global_browser_dir = None

            try:
                global_browser_dir = tempfile.mkdtemp()
                port = get_free_port()
                co = init_browser_options(port, global_browser_dir)
                mode_text = "无头模式" if headless_mode else "有头模式(调试)"
                log_to_ui("info", f"🖥️ 正在初始化浏览器: {mode_text}")
...
def cleanup_global_browser():
    """清理全局浏览器"""
    global global_browser, global_browser_dir, browser_initialized, delegated_account_active, delegated_switch_ok
...
def load_state():
    global global_token, monitor_tasks, monitor_active, processed_users, pending_results, notification_monitoring, delegated_account, history_ids, headless_mode, content_dedupe
    ensure_data_dir()
